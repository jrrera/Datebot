...
(function($){ 	//Sets the document as jQuery
	...
	var listEmOut = function(babesArr){           
		...
		for (var i = 0; i < babesArr.length; i++) {
			...
			babeOutput += '<td class="context"> <ul> ' + processContext(babesArr[i]) + ' </ul> </td><td class="message" id="' + babesArr[i].user + '"><div id="' + babesArr[i].user + 'message"> ' + processMessage(babesArr[i]) + ' <p>Jon</p></div><button class="edit">Edit</button><textarea wrap="hard"></textarea><button class="save">Save</button></td>';
	...
	var updateMessage = function(babesArr) {
		$(window).load(function(){  
			...
			if (clickedName != "manual" && clickedName != "reject") { //Prevent code from being run if the class of input being clicked was the 'reject' or 'manual message' button
				...
				$('#'+clickedName+'message').html(finalMessageUpdate + '<p>Jon</p>'); //Now, identify the correct td to replace with new HTML
			}
	 		}); 
    ...
    }); //Alex-written code that captures JSON file from API call
